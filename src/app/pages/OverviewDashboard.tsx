...
export function OverviewDashboard() {
  const { tables, queue, reservations, tattooReservations } = useAppContext();
  ...
  const recentTattoos = tattooReservations
    .filter(r => r.status !== 'cancelled')
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    .slice(0, 4);
  const tattooDepositPending = tattooReservations.filter(r => !r.depositPaid && r.status !== 'cancelled').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
          {tables.filter(t => t.isActive).map(t => {
            ...
            // Find the nearest upcoming reservation for this table today
            const nextRes = reservations
              .filter(r => r.tableId === t.id && (r.status === 'pending' || r.status === 'confirmed') && isToday(new Date(r.date)))
            ...
            return (
              <div
                ...
                className={`aspect-square rounded-lg flex flex-col items-center justify-center cursor-pointer border transition-all text-center p-1
                ...
                `}
              >
                <span className="text-[11px] font-bold truncate w-full leading-tight">{t.name}</span>
                
                {/* Dynamic Sub-status */}
                {t.status === 'occupied' && <span className="text-[9px] opacity-80 mt-0.5 tracking-wider uppercase">In Use</span>}
                {t.status === 'available' && <span className="text-[9px] opacity-80 mt-0.5 tracking-wider uppercase">Free</span>}
                {t.status === 'reserved' && (
                  <span className="text-[9px] opacity-90 mt-0.5 font-medium tracking-wider">
                    {nextRes ? `@ ${nextRes.timeSlot}` : 'Reserved'}
                  </span>
                )}
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Queue Snapshot + Tattoo Reservations */}
        <div className="space-y-4">
          ...
          {/* Tattoo Reservations snapshot */}
          <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-neutral-300 flex items-center gap-2">
                <Palette size={15} className="text-violet-400" /> Tattoo Reservations
                {tattooDepositPending > 0 && <span className="bg-rose-500 text-white text-[10px] font-black rounded-full w-4 h-4 flex items-center justify-center">{tattooDepositPending}</span>}
              </h2>
              <button onClick={() => navigate('/staff/tattoo-reservations')} className="text-xs text-emerald-500 hover:text-emerald-400 flex items-center gap-1 font-semibold">
                View All <ChevronRight size={12} />
              </button>
            </div>
            {recentTattoos.length === 0 ? (
              <div className="flex items-center gap-2 py-2">
                ...
                <p className="text-sm text-neutral-500">No tattoo reservations</p>
              </div>
            ) : (
              <div className="space-y-2">
                {recentTattoos.map(r => {
                  const isUpcoming = isFuture(new Date(r.date));
                  const isDateToday = isToday(new Date(r.date));
                  return (
                    <div key={r.id} className="flex items-center gap-2.5 bg-neutral-900/60 rounded-lg px-3 py-2 border border-neutral-800/60">
                      <div className="w-7 h-7 rounded-full bg-violet-600/20 border border-violet-600/30 flex items-center justify-center text-xs font-bold text-violet-400 flex-none">
                        {r.customerName.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-neutral-200 truncate">{r.customerName}</p>
                        <p className="text-[10px] text-neutral-500 truncate">
                          {r.artistName} · {isDateToday ? 'Today' : isTomorrow(new Date(r.date)) ? 'Tomorrow' : format(new Date(r.date), 'MMM d')}, {r.timeSlot}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-0.5 flex-none">
                        ...
                        {!r.depositPaid && <span className="text-[9px] text-rose-400">no deposit</span>}
                      </div>
                    </div>
                  );
                })}
