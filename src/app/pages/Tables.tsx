...
export function TableCard({ table, onAssign, onEnd, onExtend, nextReservation }: TableCardProps) {
  ...
  const getTimerInfo = () => {
    ...
    // 🚨 FIXED: "Open Time" Feature Integration
  ...
  const getBorderColor = () => {
    ...
    if (timer?.isOpenTime) return 'border-blue-800/40'; // 🚨 Blue border for Open Time
    ...
    <div className={clsx(
      ...
      <div className="flex-1 flex flex-col justify-end gap-2">
        {table.status === 'occupied' && table.session && timer ? (
          ...
          <button
            onClick={onAssign}
            className="w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-neutral-800 rounded-lg hover:border-emerald-600/50 hover:bg-emerald-950/10 transition-all group cursor-pointer"
          >
