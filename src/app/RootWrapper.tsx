...
function RootContent() {
  ...
  // Swap between Billiards and Drinks every 2.5 seconds
  useEffect(() => {
    if (displayState !== 'loading') return;
    const interval = setInterval(() => {
      setAnimIndex(prev => (prev + 1) % 2);
    }, 2500);
    return () => clearInterval(interval);
  }, [displayState]);

  // 🚨 FIX 1: Trigger the slogan when loading finishes
  useEffect(() => {
  ...
  // 🚨 FIX 2: Handle the timer completely separately so it doesn't get cancelled!
  useEffect(() => {
    if (displayState === 'slogan') {
      const timer = setTimeout(() => {
        setDisplayState('done');
      }, 2500); // Hold the slogan on screen for 2.5 seconds
                ...
                {animIndex === 0 ? (
                  ...
                  >
                    <div className="relative w-32 h-32 flex items-center justify-center">
                      ...
                      {/* Left Glass */}
                      <motion.div
                        className="absolute flex flex-col items-center origin-bottom z-10"
                        animate={{ x: [-30, -10, -30], rotate: [-15, 12, -15] }}
                        transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
                      >
                        <div className="w-6 h-12 border-2 border-amber-100/50 rounded-b-full relative overflow-hidden flex items-end pb-1">
                          <div className="w-full h-[70%] bg-amber-400/70 relative">
                            <div className="absolute bottom-1 left-1 w-1 h-1 bg-white/80 rounded-full animate-[ping_1s_infinite]" />
                            <div className="absolute bottom-3 right-1 w-1 h-1 bg-white/80 rounded-full animate-[ping_1.2s_infinite_0.2s]" />
                          </div>
                        </div>
                        <div className="w-1 h-6 bg-amber-100/50" />
                        <div className="w-5 h-1 bg-amber-100/50 rounded-t-sm" />
                      </motion.div>
                      ...
                      >
                        <div className="w-6 h-12 border-2 border-amber-100/50 rounded-b-full relative overflow-hidden flex items-end pb-1">
                          <div className="w-full h-[70%] bg-amber-400/70 relative">
                            <div className="absolute bottom-2 left-1 w-1 h-1 bg-white/80 rounded-full animate-[ping_1.1s_infinite]" />
                            <div className="absolute bottom-1 right-1 w-1 h-1 bg-white/80 rounded-full animate-[ping_1.3s_infinite_0.4s]" />
                          </div>
                        </div>
                        <div className="w-1 h-6 bg-amber-100/50" />
                        <div className="w-5 h-1 bg-amber-100/50 rounded-t-sm" />
                      ...
                      {/* The Clink Spark */}
                      <motion.div
                        className="absolute top-8 w-6 h-6 bg-amber-200 rounded-full blur-[3px] z-20 mix-blend-screen"
